...
namespace westcoast_cars.api.Controllers
{
    ...
    public class TransmissionsController : ControllerBase, IBaseApiController
    {
        ...
        {
            if (await _context.TransmissionTypes.SingleOrDefaultAsync(
                 f => f.Name.ToLower().Trim() == name.ToLower().Trim()) is not null)
            {

                return BadRequest($"Det finns redan en växellåde typ med namnet {name}");
            }
            ...
            if (await _context.SaveChangesAsync() > 0)
            {
                return CreatedAtAction(nameof(GetById),
                    new { id = gearType.Id },
                    new { Id = gearType.Id, Name = gearType.Name });
            }
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            _context.TransmissionTypes.Remove(gearType);
